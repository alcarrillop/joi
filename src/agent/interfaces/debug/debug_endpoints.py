        ...
        return [
            {
                "id": str(assessment["id"]),
                "timestamp": assessment["timestamp"],
                "user_message": assessment["user_message"][:100] + "..."
                if len(assessment["user_message"]) > 100
                else assessment["user_message"],
    ...
    try:
        ...
        if not detection:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=404, detail=f"No level detection found and unable to generate: {str(e)}"
                )
...
async def get_assessment_system_summary():
    ...
    try:
        ...
        return {
            ...
            "level_distribution": [
                {"level": row["detected_level"], "count": row["count"]} for row in level_distribution
            ],
            "skill_statistics": [
                {
                    "skill": row["skill"],
            ...
            "progression_trends": [{"trend": row["trend"], "count": row["count"]} for row in recent_trends],
        }
    except Exception as e:
    ...
    try:
        ...
        if not assessment:
            return {
                ...
                "debug_info": {"message_length": len(message.split()), "debug_mode": True},
            }
        ...
        return {
            ...
            "feedback": {"strengths": assessment.strengths, "areas_for_improvement": assessment.areas_for_improvement},
