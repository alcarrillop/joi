...
class LearningStatsManager:
    ...
    async def get_vocabulary_insights(self, user_id: str) -> Dict[str, Any]:
        """Get vocabulary insights and basic complexity metrics."""
        try:
            words = await self.db_ops.get_all_user_words(user_id)

            if not words:
                return {"total_vocabulary": 0, "insights": "Not enough data yet"}

            # Categorize words by complexity
            basic_words = []
            intermediate_words = []
            advanced_words = []

            for word, freq in words:
                if word in self.basic_vocabulary:
                    basic_words.append(word)
                elif word in self.intermediate_vocabulary:
            ...
            vocab_diversity = {
                "basic_words": len(basic_words),
                "intermediate_words": len(intermediate_words),
                "advanced_words": len(advanced_words),
                "total_words": total_words,
            }

            avg_word_length = sum(len(word) for word, _ in words) / total_words if total_words > 0 else 0

            complexity_score = (
                (len(basic_words) * 1 + len(intermediate_words) * 2 + len(advanced_words) * 3) / total_words
                if total_words > 0
                else 0
            )
            ...
            return {
                ...
                "vocabulary_distribution": vocab_diversity,
                "avg_word_length": round(avg_word_length, 1),
                "complexity_score": round(complexity_score, 2),
                "insights": self._generate_vocabulary_insights(vocab_diversity, complexity_score),
            }

        except Exception as e:
            self.logger.error(f"Failed to get vocabulary insights for user {user_id}: {e}")
            return {"error": str(e), "total_vocabulary": 0}

    def _generate_vocabulary_insights(self, distribution: Dict[str, int], complexity: float) -> str:
        """Generate helpful insights based on vocabulary analysis."""
        total = distribution["total_words"]
        basic_pct = (distribution["basic_words"] / total * 100) if total > 0 else 0
        advanced_pct = (distribution["advanced_words"] / total * 100) if total > 0 else 0

        insights = []

        if basic_pct > 70:
            insights.append("You're building a strong foundation with basic vocabulary!")
        elif basic_pct < 40:
            insights.append("Consider practicing more basic everyday words.")

        if advanced_pct > 30:
            insights.append("Excellent use of sophisticated vocabulary!")
        elif advanced_pct > 15:
            insights.append("Good progress with intermediate-level words.")

        if complexity > 2.5:
            insights.append("Your vocabulary shows advanced language skills.")
        elif complexity > 1.8:
            insights.append("You're developing solid intermediate vocabulary.")
        else:
            insights.append("Focus on expanding beyond basic vocabulary.")

        return " ".join(insights) if insights else "Keep practicing to build your vocabulary!"
