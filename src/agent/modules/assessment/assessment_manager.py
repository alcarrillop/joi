...
from .models import (
    ConversationAssessment, LevelDetectionResult, SkillProgression,
    LanguageSkill, AutoAssessmentConfig
)
from .analyzers import VocabularyAnalyzer, GrammarAnalyzer, FluencyAnalyzer
...
def get_curriculum_models():
    from src.agent.modules.curriculum.models import CEFRLevel, AssessmentResult, SkillType
...
class AssessmentManager:
    ...
    async def assess_user_message(
        ...
        skills_scores = self._calculate_skills_scores(
            vocabulary_analysis, grammar_analysis, fluency_analysis
        )
    ...
    def _calculate_skills_scores(self, vocab_analysis, grammar_analysis, fluency_analysis) -> Dict[LanguageSkill, float]:
    ...
    async def _identify_competency_evidence(
        ...
        try:
            ...
            for competency in current_competencies:
                ...
                # Vocabulary evidence
                vocab_matches = sum(1 for word in vocab_analysis.words_used 
    ...
    async def _store_assessment(self, assessment: ConversationAssessment):
        ...
        try:
            ...
            try:
                ...
                """, 
                    str(uuid.uuid4()), assessment.user_id, assessment.session_id, 
                    assessment.message_id, assessment.timestamp, assessment.user_message,
                    assessment.overall_level, assessment.confidence_score,
            ...
            for competency_id, evidence_score in assessment.competency_evidence.items():
                if evidence_score > 0.7:  # High threshold for automatic assessment
                    ...
                    curriculum_assessment = AssessmentResult(
                        ...
                        feedback=f"Automated assessment based on conversation analysis",
