...
from .models import AssessmentResult, CEFRLevel, Competency, LearningGoal, UserProgress
...
class CurriculumManager:
    ...
    async def initialize_user_progress(self, user_id: str, initial_level: CEFRLevel = CEFRLevel.A1) -> UserProgress:
        ...
        try:
            progress = UserProgress(user_id=user_id, current_level=initial_level, level_start_date=datetime.now())
            ...
            """,
                ...
                list(progress.completed_competencies),
                list(progress.in_progress_competencies),
                json.dumps({}),
                None,
            )
    ...
    async def get_current_competencies(self, user_id: str) -> List[Competency]:
        ...
        unstarted = [
            c
            for c in available_competencies
            if c.id not in progress.completed_competencies and c.id not in progress.in_progress_competencies
        ]
        ...
        if unstarted:
            ...
            return [c for c in available_competencies if c.id in progress.in_progress_competencies]
    ...
    async def get_next_recommended_competency(self, user_id: str) -> Optional[Competency]:
        ...
        not_started = [
            ...
            if comp.id not in progress.completed_competencies and comp.id not in progress.in_progress_competencies
        ]
        ...
        # If no unstarted competencies, review in-progress ones
        in_progress = [comp for comp in available_competencies if comp.id in progress.in_progress_competencies]
    ...
    async def _update_user_progress(self, assessment: AssessmentResult):
        ...
        try:
            ...
            """,
                list(progress.completed_competencies),
                list(progress.in_progress_competencies),
                json.dumps(progress.mastery_scores),
    ...
    async def get_learning_statistics(self, user_id: str) -> Dict:
        ...
        try:
            ...
            avg_score = (
                sum(progress.mastery_scores.values()) / len(progress.mastery_scores) if progress.mastery_scores else 0
            )
            ...
            # Averages by skill
            skill_averages = {skill: sum(scores) / len(scores) for skill, scores in skill_scores.items()}
            ...
            return {
                ...
                "time_in_current_level": (datetime.now() - progress.level_start_date).days
                if progress.level_start_date
                else 0,
                "recent_assessments": [
                    {
