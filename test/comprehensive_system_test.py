...
class ComprehensiveSystemTest:
    ...
    async def test_database_connectivity(self) -> bool:
        ...
        try:
            ...
            # Test basic query
            await conn.fetchval("SELECT current_database(), current_user")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if all_healthy:
                        print(
                            "   💚 All components healthy: database, vector_store, memory_manager, curriculum_manager"
                        )
                        return True
                    else:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await client.get(
                    f"{self.base_url}/debug/users/{self.test_user_id}/memories?query=girlfriend"
                )
                if response.status_code == 200:
                ...
                response = await client.post(
                    f"{self.base_url}/debug/users/{self.test_user_id}/test-memory?query=what%20do%20you%20remember%20about%20me"
                )
                if response.status_code == 200:
                    test_result = response.json()
                    memories_found = test_result.get("memories_found", 0)
                    print(
                        f"   🧪 Memory test with 'what do you remember about me' found {memories_found} relevant memories"
                    )
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                curriculum_response = await client.get(
                    f"{self.base_url}/debug/users/{self.test_user_id}/curriculum-progress"
                )

                if assessments_response.status_code == 200 and curriculum_response.status_code == 200:
                    assessments = assessments_response.json()
                    print(
                        f"   🔗 Assessment-Curriculum integration: {len(assessments)} assessments, curriculum available"
                    )
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 400:  # Should return bad request
                    ...
                    print("   ✅ Invalid UUIDs rejected, non-existent users handled gracefully")
                    return True
                else:
                    return False
            except Exception as e:
    ...
    async def test_performance(self) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                endpoints = ["/debug/stats", f"/debug/users/{self.test_user_id}/memories?query=test", "/debug/health"]
                ...
                if all(time < 5.0 for time in times):
                    print("   ✅ All response times within acceptable limits")
                    return True
                else:
